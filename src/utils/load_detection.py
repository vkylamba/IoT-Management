...
from device.clickhouse_models import (Meter, MeterData, MeterLoad, WeatherData,
...
def get_load_data_ai(device, data_point, sorted_equipments, temperature, humidity, wind_speed):
    """
        Method to find out appaliances list based on the data.
    """
    equipments = []
    ...
    for load in sorted_equipments:
        ...
        if load.equipment.name in load_model.targets:
            try:
                ...
                logger.exception(f"Exception occured while checking for load {load.equipment.name}", ex)
    return equipments
