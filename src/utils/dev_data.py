...
from device.clickhouse_models import DerivedData, Meter, MeterData, RawData, MeterLoad, WeatherData
from device.models import DeviceProperty, DeviceType, DeviceEquipment, device, DeviceStatus
...
class DataReports(object):
    ...
    def get_statistics_by_time(self, params='all', from_time=None, to_time=None, aggregation_period=None):
        ...
        rows = connection.raw("""
            ...
            from
                meter as meter
                join meterdata as data on meter.id = data.meter
            where
                meter.device = {device_id}
    ...
    def get_power_and_energy_data(self, start_time, end_time, meter_type=None):
        ...
        rows = connection.raw(
            """
                ...
                from
                    meterdata as data
                join meter on meter.id = data.meter
                where
                    data.meter in {meter_ids}
                    and data.data_arrival_time >= toDateTime('{from_time_string}')
                    and data.data_arrival_time <  toDateTime('{to_time_string}')
                group by
                    meter.name, data_arrival_time
                order by
                    data_arrival_time
            """.format(
                meter_ids=meter_ids,
