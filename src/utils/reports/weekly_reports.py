...
def get_statistics_last_week(dr, from_time, to_time, params='all'):
    ...
    return dr.get_statistics_by_time(
        ...
        aggregation_period=datetime.timedelta(days=7)
    )
...
def get_weekly_report(device):
    dr = DataReports(device)
    
    local_time = dr.get_device_local_time()

    to_time = local_time - datetime.timedelta(days=local_time.weekday())
    ...
    last_month_data = [meter_data for meter_data in get_statistics_last_week(dr, from_time, to_time)]
