...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argement('deivce_type', nargs='+', type=str)
    ...
    def handle(self, *args, **options):
        ...
        if len(device.meter_set.all()) == 0:
            meter = Meter(
                name=f"{device.ip_address}-ac-meter",
                device=device,
                meter_type=Meter.AC_METER
            )
            meter.save()
        ...
        while (end_time - start_time).total_seconds() > 0:
            ...
            # )
            # if not hasattr(data_obj, 'weatherdata'):
            #     data_obj.weatherdata = WeatherData()
            # data_obj.weatherdata.temperature = random.randrange(100, 500)
            # data_obj.weatherdata.humidity = random.randrange(20, 50)
            # data_obj.weatherdata.wind_speed = random.randrange(0, 100)
            # data_obj.weatherdata.save()
